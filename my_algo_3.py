...
class TrainModel:
	...
	def train(self, X_train, y_train, X_val, X_test,y_test, c_weight):
		...
		print("Train shape: ",self.train_y_predicted1.shape)

		self.test_y_predicted = ensemble_probas_train(self.train_y_predicted1,self.train_y_predicted2,self.train_y_predicted3,y_train,self.test_y_predicted1,self.test_y_predicted2,self.test_y_predicted3,y_test)
...
# ================== Function for ensembling the probabilities of train and returning test probabs ===========
def ensemble_probas_train(probas_train1,probas_train2,probas_train3,train_y,probas_test1,probas_test2,probas_test3,test_y):
	res_train = combine_probas(probas_train1,probas_train2,probas_train3)
	res_test = combine_probas(probas_test1,probas_test2,probas_test3)
	return nn3.train(trn_x = res_train,trn_y = train_y,tst_x = res_test, tst_y = test_y,ep = tot_epochs,num_class = n_class)
# ====================================================================================================

# ================== Function for ensembling the probabilities of validation and return probabs ===========
def ensemble_probas_val(probas_val1,probas_val2,probas_val3):
	res_val = combine_probas(probas_val1,probas_val2,probas_val3)
	return nn3.validate(validate_x = res_val)
# ====================================================================================================

# ================== Function for combining the probabilities =======================================
def combine_probas(probas_val1,probas_val2,probas_val3):
...
class TheAlgorithm(object):
	...
	def run(self, X_train_full, y_train_full, X_test, y_test):
		...
		(X_train, X_val, X_test) = self.clf_model.train(X_train, y_train, X_val, X_test,y_test, 'balanced')
		active_iteration = 1
		...
		while self.queried < max_queried:
			...
			probas_val = ensemble_probas_val(probas_val1=probas_val1,probas_val2=probas_val2,probas_val3=probas_val3)
			# =======================================================================================
...
tot_epochs = 10
n_class = 10
Ks = [50] # Number of samples to select each time
repeats = 1
